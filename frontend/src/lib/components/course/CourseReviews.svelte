...
<section aria-labelledby={`${id}-title`}>
	<div class="flex flex-wrap items-baseline justify-between gap-2">
		<h3
			...
			id={`${id}-title`}
			tabindex="-1"
			class="text-sm font-semibold text-text-primary focus:outline-none"
		>
			{m.reviews_title()}
		</h3>
		{#if data}
	...
	{#if notice}
		<p role="status" class="mt-3 text-sm text-text-primary">
			{notice === 'saved' ? m.reviews_saved() : m.reviews_deleted()}
		</p>
	{/if}
	{#if error}
		<div
			...
			role="alert"
			tabindex="-1"
			class="mt-3 rounded-lg border border-red-500 p-3 text-sm text-red-700 focus:outline-none dark:text-red-300"
		>
			<p>{errorMessage}</p>
			{#if error === 'session'}
				<button
					...
					onclick={signIn}
					disabled={busy !== null}
					class="mt-2 min-h-11 rounded-lg border border-border-secondary px-3 font-medium text-text-primary disabled:opacity-50"
				>
	...
	{#if loading}
		<p role="status" class="mt-3 text-sm text-text-secondary">
			{m.reviews_loading()}
		</p>
	{:else if loadFailed}
		...
		<button
			type="button"
			onclick={() => load()}
			class="mt-2 min-h-11 rounded-lg border border-border-secondary px-3 text-sm text-text-primary"
		>
			{m.common_retry()}
		</button>
	{:else if data}
		...
		<div class="mt-4">
			{#if !user}
				...
				<button
					...
					onclick={signIn}
					disabled={busy !== null}
					class="mt-2 min-h-11 rounded-lg border border-border-secondary bg-bg-primary px-4 text-sm font-medium text-text-primary hover:bg-bg-secondary focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-500 disabled:opacity-50"
				>
					{m.account_continue_github()}
				</button>
			{:else if editing}
			...
			{:else}
				<div class="flex flex-wrap gap-2">
					<button
						...
						type="button"
						disabled={busy !== null}
						onclick={() => { editing = true; error = null; notice = null; }}
						class="min-h-11 rounded-lg border border-border-secondary bg-bg-primary px-4 text-sm font-medium text-text-primary hover:bg-bg-secondary focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue-500 disabled:opacity-50"
					>
						{ownReview ? m.reviews_edit() : m.reviews_write()}
					</button>
					{#if ownReview}
						<button
							type="button"
							...
							class="min-h-11 rounded-lg px-3 text-sm text-red-700 hover:bg-red-500/10 focus-visible:outline focus-visible:outline-2 focus-visible:outline-red-500 disabled:opacity-50 dark:text-red-300"
						>
							{m.reviews_delete()}
						</button>
					{/if}
		...
		{#if busy === 'deleting'}
			<p role="status" class="mt-2 text-sm text-text-secondary">
				{m.reviews_deleting()}
			</p>
		{/if}
			...
			{#each data.reviews as review (review.id)}
				<li>
					...
					>
						...
						{#if review.text}
							<p
								class="mt-2 whitespace-pre-wrap break-words text-sm leading-relaxed text-text-primary [overflow-wrap:anywhere]"
							>
								{review.text}
							</p>
						{/if}
						{#if review.updatedAt > review.createdAt}
							<p class="mt-2 text-xs text-text-secondary">
								{m.reviews_updated({ date: dateFormat.format(review.updatedAt) })}
							</p>
						{/if}
					</article>
				</li>
...
{#if confirmingDelete}
	<ConfirmationDialog
		title={m.reviews_delete_title()}
		message={m.reviews_delete_message()}
		confirmText={m.reviews_delete()}
		onConfirm={remove}
