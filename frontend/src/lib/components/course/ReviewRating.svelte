	...
	<div class="flex gap-1">
		{#each [1, 2, 3, 4, 5] as score (score)}
			...
			>
				...
				>
				<span
					class="flex h-full w-full items-center justify-center rounded-lg border text-sm peer-focus-visible:outline peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-blue-500 peer-disabled:opacity-50 {highlighted ? 'border-blue-600 bg-blue-600 text-white' : 'border-border-secondary bg-bg-primary text-text-secondary'}"
				>
					{#if stars}
						<svg
							class="h-6 w-6"
