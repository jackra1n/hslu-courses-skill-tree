...
test('reviews sort by recommendation with newest ties, or by publication date', async ({
	page,
}) => {
	const entries = [
		{ text: 'Newest critical review', recommendation: 1, day: 4 },
		{ text: 'Recent positive review', recommendation: 5, day: 3 },
		{ text: 'Earlier positive review', recommendation: 5, day: 2 },
		{ text: 'Oldest mixed review', recommendation: 3, day: 1 },
	];
	await page.route('**/api/courses/WEBLAB/reviews', (route) =>
		route.fulfill({
			json: {
				reviews: entries.map(({ text, recommendation, day }) => ({
					id: `review-${day}`,
					courseId: 'WEBLAB',
					text,
					recommendation,
					...
					workload: 3,
					createdAt: Date.UTC(2026, 0, day),
					updatedAt: Date.UTC(2026, 0, day),
				})),
				ownReviewId: null,
				summary: {
					count: 4,
	...
	const articles = reviews.getByRole('article');
	const sort = reviews.getByRole('combobox', {
		name: 'Sort reviews',
		exact: true,
	});
	await expect(articles).toContainText([
		'Recent positive review',
		'Earlier positive review',
		'Oldest mixed review',
		'Newest critical review',
	]);
	await sort.click();
	await page
		.getByRole('option', { name: 'Lowest recommendation', exact: true })
		.click();
	await expect(articles).toContainText([
		'Newest critical review',
		'Oldest mixed review',
	...
	await expect(articles).toContainText(entries.map(({ text }) => text));
	await sort.click();
	await page.getByRole('option', { name: 'Oldest first', exact: true }).click();
	await expect(articles).toContainText(
		entries.toReversed().map(({ text }) => text),
	);
});
