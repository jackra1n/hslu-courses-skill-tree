...
const SNAPSHOT = {
	...
	studyPlans: {},
	slotStatus: {},
	preferences: { showShortNamesOnly: false, showCourseTypeBadges: false, theme: 'light' }
};
...
describe('worker /api router', () => {
	it('returns 401 for /api/progress without a session', async () => {
		const res = await dispatch(request('GET', 'https://hsluskilltree.com/api/progress'));
	...
	it('returns 405 with Allow header for wrong methods on /api/progress', async () => {
		const res = await dispatch(request('POST', 'https://hsluskilltree.com/api/progress'));
		expect(res.status).toBe(405);
		expect(res.headers.get('Allow')).toBe('GET, PUT');
	});

	it('returns 404 JSON for unknown /api routes', async () => {
		const res = await dispatch(request('GET', 'https://hsluskilltree.com/api/unknown'));
		expect(res.status).toBe(404);
		expect(res.headers.get('Content-Type')).toContain('application/json');
	});

	it('serves /api/auth/get-session without a server error', async () => {
		const res = await dispatch(request('GET', 'https://hsluskilltree.com/api/auth/get-session'));
	...
	it('creates no user_data row for anonymous traffic', async () => {
		...
		const rows = await testEnv.DB.prepare('SELECT COUNT(*) AS count FROM user_data').first<{
			count: number;
		}>();
