...
async function readBoundedBody(request: Request): Promise<string | null> {
	...
	let out = '';
	for (const chunk of chunks) out += new TextDecoder().decode(chunk, { stream: true });
...
function parsePutBody(
	...
	const { data, expectedRevision } = parsed as { data: unknown; expectedRevision: unknown };
	...
	return { ok: true, body: { data, expectedRevision: expectedRevision as number | null } };
}
...
export async function handleProgressRequest(
	...
	if (request.method === 'GET' && url.pathname === '/api/progress') {
		...
		try {
			return json({ data: JSON.parse(row.data), revision: row.revision, updatedAt: row.updated_at }, 200);
		} catch {
	...
	if (request.method === 'PUT' && url.pathname === '/api/progress') {
		...
		if (changed === 0) {
			...
			try {
				return json(
					{ data: JSON.parse(current.data), revision: current.revision, updatedAt: current.updated_at },
					409
				);
			} catch {
				return json({ error: 'internal' }, 500);
		...
		return json({ data, revision: expectedRevision === null ? 1 : expectedRevision + 1, updatedAt }, 200);
	}
